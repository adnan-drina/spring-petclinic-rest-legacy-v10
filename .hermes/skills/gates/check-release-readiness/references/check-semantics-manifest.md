...
| `mta_rescan` | `assert-mta-rescan.py` over `verification/mta-rescan/findings.json` (analyzer_ran; `tree_sha256` = the tree on disk; `normalized_at` newer than the last loop commit, dated by git; `input_digest` ≠ M1 snapshot) | The analyzer ran over the tree M4 judges, after M3 | **SEMANTIC (analysis)** | File-present / findings-handoff is not a rescan (v19 WC-5); the legacy `evidence/mta-findings.json` compared with its own M1 snapshot is not a rescan either (v9 t_caf2ad51) | `analyzer_ran: true`, `tree_sha256` equal to this tree's, stamp after the last M3 commit, `input_digest` ≠ M1 snapshot |
